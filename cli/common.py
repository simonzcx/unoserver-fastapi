...
# [修改] 合并所有支持的文件类型
all_supported_suffixes = pdf_suffixes + image_suffixes + word_suffixes
...
def read_fn(path):
    ...
    with open(str(path), "rb") as input_file:
        ...
        if file_suffix in image_suffixes:
            ...
            return file_bytes
        # [修改] 移除Word文件的异常处理，因为现在应该在上层处理
        else:
            # 如果不是PDF或图像，检查是否是Word文件
            file_ext = path.suffix.lower().lstrip('.')
            if file_ext in word_suffixes:
                raise Exception(f"Word file not converted to PDF: {file_ext}. Please convert to PDF first.")
            else:
